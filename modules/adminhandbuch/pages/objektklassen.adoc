...
Objektklassen fungieren als Vorlage bei der Generierung von Objekten. Die Zuordnung eines Objekts zu einer Objektklasse ermöglicht weiter klassenbezogene Funktonen und Abfragen. Die Objektklasse dient damit als Ordnungs- und Zuordnungskriterium. 
...
Um Inhalte einer Objektklasse zu pflegen, klickt man auf den Namen der Objektklasse in der Liste. Es öffnet sich die Objektklassenbearbeitung. Die Inhalte lassen sich entsprechend der xref:anwenderhandbuch:objekte.adoc[Objektbearbeitung] pflegen und anpassen.
