...
Die Parametrierung der 2-Faktor-Authentifizierung (2FA) steht in Administratoransicht des Benutzerkontos nicht zur Verfügung. Im xref:adminhandbuch:konfiguration.adoc[Konfigurationsbereich] der Anwendung kann jedoch eine generelle Einstellung dazu vorgenommen werden, ob die 2FA verfplichtend oder optional sein soll.
...
Die Beschreibungen zu den einzelen Feldern / Attributen finden sich im Adminhandbuch im Kapitel xref:_manuell[manuellen Anlegen] und im Anwenderhandbuch im Kapitel xref:anwenderhandbuch:benutzerbezogene-seiten.adoc#_mein_konto[Mein Konto].
...
Der Tap _Objekte_ zeigt die Objekte an, in denen der Benutzer Rechte mittels einer oder mehrerer Rechte besitzt. 

image:adminhandbuch/benutzer_bearbeiten_objekte.png[Objektrollenzuordnung]

In dem Beispiel unten hat der Benutzer in einem der gelisteten Objekten gleich zwei Rollen. In diesem Fall kummulieren sich die Rechte.
...
+
Der Administrator hat aber auch hier die Möglichkeit die Zuordnung zu bearbeiten. Die Rollen, die ein Benutzer in einem der aufgeführten Objekte hat, kann über den Button image:adminhandbuch/button_bearbeiten.png[Bearbeiten] erfolgen. Es werden die Rollen gelistet und durch Setzen oder Entfernen der Häkchen werden die Rollen aktiviert oder deaktiviert. +
+
