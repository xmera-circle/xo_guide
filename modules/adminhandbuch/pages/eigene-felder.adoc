...
Eigene Felder bieten die Möglichkeit für den Großteil der Objekttypen (Objekte / Module / Masken) individuelle Felder / Attribute hinzuzufügen und damit Informationen zu dokumentieren, die im xmera Standard nicht implementiert sind.

Verwaltet werden _Eigene Felder_ im Administrationsbereich im Menüpunkt _Eigene Felder_. Über dieses Menü gelangt man in die Listenansicht der _Eigene Felder_.
...
Für jeden Objekttyp, für den zumindest ein eigenes Feld angelegt wurde, ist in der Listenansicht ein Reiter vorhanden. Wird ein Objekttyp im Reiter ausgewählt, werden unterhalb die dem Objekttyp zugeordneten _Eigenen Felder_ angezeigt.
...
|Kombinationsmatrix
|Die Kombinationsmatrix definiert das Ergebnis einer Kombination aus zwei Feldern mit dem Format Eigenschaft/Wert-Paar. Jeder Wertekombination kann ein Ergebnis (Text) und eine Farbe zugeordnet werden. Der aufgespannte Ergebnisraum ergibt sich über das Mapping der Berechnung definiert.

|Langer Text
|Ein in seiner Größe änderbarer mehrzeiliger Text.

|Link
|Ein URL-Link kann erfasst werden.

|Liste
|Aus einer Liste, die im _Eigenen Feld_ definiert wird kann ein oder mehrere Wert(e) gewählt werden.

|Paket
|Aus einer Liste der Pakete können eine oder mehrere Paket(e) gewählt werden.
...
[NOTE]
Nicht für jeden Objekttyp stehen alle Feldformate zur Verfügung.
...
Bei Auswahlfeldern (z.B. Listen, Benutzern oder Paketen) ist im Standard nur ein Wert erlaubt. Über Anhaken der Auswahl _Mehrere Werte_ kann auf Mehrfachauswahl umgestellt werden. +
Weiter kann bei der _Anzeige_ der Auswahl zwischen der Dropdown-Liste und der Checkbox gewählt werden.

Abhängig vom Objekttyp, dem das _Eigene Feld_ zugeordnet wird, besteht, die Nutzung des Feldes einzuschränken. Mögliche Filter sind
...
Bei eigenen Feldern gibt zu jedem Feld aus der Listenansicht 3 Bearbeitungsoptionen.
...
Um die Erstellung von eigenen Feldern zu erleichtern, kann auch ein bestehendes eigenes Feld zu kopieren und dieses anzupassen. Beim Kopieren (Klick auf den Button image:adminhandbuch/button_kopieren.png[Eigenes Feld kopieren]) wird bis auf den Namen die Konfiguration des Ausgangsfelds übernommen. +
+
Mit Klick auf den Button image:adminhandbuch/button_anlegen.png[Eigenes Feld anlegen] wird das neue Feld erzeugt.

Löschen::

Durch Klick auf den Button image:adminhandbuch/button_loeschen.png[Eigenes Feld löschen] wird nach der Bestätigung in einem Pop-Up-Fenster das Eigene Fled gelöscht. +
+
[WARNING]
