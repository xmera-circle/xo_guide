...
Anschließend gelangt man auf die Hauptseite von xmera Omnia. Diese beinhaltet das xref:dashboard.adoc[xmera Omnia Dashboard].
...
- *Objekte:* Zugang zu den xref:objekte.adoc[Objekten] (Projekten, Prozessen, Assets)
- *DMS:* Zugang zum xref:modul-dms.adoc[Dokumenten-Management-System]
...
Mit der Möglichkeit die so gestalteten Listen als Abfragen (blau)zu speichern und anderen Nutzern zugänglich zu machen, ermöglicht es, Listen mit verschiedensten Sichten auf die Daten zu gestalten und somit einen schnellen Überblick auf die benötigten Informationen zu erhalten. Über den Button image:anwender-handbuch/button_zuruecksetzen.png[Zurücksetzen] wird der Filter wieder auf die Standard-Liste zurückgesetzt.
...
Im Filterbereich der Liste, der durch den Pfeil image:anwender-handbuch/button_ausklappen.png[] ausgeklappt bzw. image:anwender-handbuch/button_einklappen.png[] eingeklappt werden. In den Zeilen im Filterbereich sind die Filter definiert.
...
Die angepassten Regeln werden durch Klick auf den Button image:anwender-handbuch/button_anwenden_grau.png[Anwenden] angewendet.
...
+
Wird nach einem Wert gefiltert so kann der gewünschte Wert ausgewählt werden. Durch Klick auf image:anwender-handbuch/button_plus.png[] bzw. image:anwender-handbuch/button_minus.png[] kann die Mehrfachauswahl aktiviert werden. +
+
...
+
Die neu erstllte Regel wird durch Klick auf den Button image:anwender-handbuch/button_anwenden_grau.png[Anwenden] angewendet.
...
In welcher Form die Liste sortiert wird, wird über das Symbol image:anwender-handbuch/einfuehrung_sortierung_up.png[] für aufsteigend bzw. image:anwender-handbuch/einfuehrung_sortierung_down.png[] für absteigend sortiert links neben der Spaltenüberschrift der Spalte, nach der sortiert wurde, dargestellt. +
+
...
+
Nach dem eine Liste erstellt wird erscheint der Button image:anwender-handbuch/button_speichern.png[]. Wird das Speichern geklickt erscheint die Abfrage-Einstellungsmaske. +
+
...
+
Mit Klick auf den Button image:anwender-handbuch/button_speichern.png[] wird die Absprache gespeichert und es erscheint die Abfrageliste.

Abfrage bearbeiten::

Um eine Abfrage ist diese aufzurufen.Anschließend kommt man durch den Klick auf den Button image:anwender-handbuch/button_bearbeiten.png[Bearbeiten] in die Eigenschaftsmaske der Abfrage, in der die Filter und Gestaltungsoptionen angepasst werden können. Mit dem speichern werden die Anpassungen übernommen und die geänderte Abfrage aufgerufen.

Abfrage löschen::

Zum Löschen einer Anfrage ist die zu löschende Anfrage zu öffnen. Durch Klicken auf den Button image:anwender-handbuch/button_loeschen.png[] wird nach Bestätigung in einer Sicherheitsabfrage die Anfrage gelöscht.